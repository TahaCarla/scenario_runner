...
from srunner.utilities.statistics_manager import StatisticsManager
from srunner.utilities.route_indexer import RouteIndexer
from srunner.autoagents.agent_wrapper import AgentWrapper
from srunner.autoagents.sensor_interface import SensorInterface, SensorConfigurationInvalid
...
class ScenarioEvaluator(object):
    ...
    def _load_and_run_scenario(self, config):
        ...
        if hasattr(config, 'index'):
            # Use the existing index for route-based scenarios
            route_index = config.index
        else:
            # Assign a default index for OpenScenario-based scenarios
            route_index = 0  # You can use a unique identifier if needed

        self.statistics_manager.set_route(config.name, route_index)
        ...
        try:
            if not self.sensors:
                ...
                AgentWrapper.validate_sensor_configuration(self.sensors, track, self._args.track)

                self.sensor_icons = [sensors_to_icons[sensor['type']] for sensor in self.sensors]
                self.statistics_manager.save_sensors(self.sensor_icons, self._args.checkpoint)
            ...
            self._register_statistics(config, self._args.checkpoint, entry_status, crash_message)
            self._cleanup()
            sys.exit(-1)
        ...
        self.statistics_manager.save_record(stats_record, 0, self._args.outputDir)
    ...
    def _run_openscenario_with_agent(self):
        ...
        # route_indexer = RouteIndexer(self._args.route, config, self._args.repetitions)
        routes = self._args.route[0]
        scenario_file = config
        route_configurations = RouteParser.parse_routes_file(routes, scenario_file, None)
        config.trajectory = route_configurations[0].trajectory
        print("!!!!!!!!!!!!!!trajectory")
        print(config.trajectory)

        # if self._args.resume:
        #     route_indexer.resume(self._args.checkpoint)
        #     self.statistics_manager.resume(self._args.checkpoint)
        # else:
        #     self.statistics_manager.clear_record(self._args.checkpoint)
        #     route_indexer.save_state(self._args.checkpoint)
        #
        ...
        # global_stats_record = self.statistics_manager.compute_global_statistics(len(route_configurations))
        # self.statistics_manager.save_global_record(global_stats_record, self.sensor_icons,
        #                                            total_routes=len(route_configurations),
        #                                            endpoint=self._args.outputDir)
        self._cleanup()
        return result
...
def main():
    ...
    result = True
    try:
        scenario_runner = ScenarioEvaluator(arguments)
